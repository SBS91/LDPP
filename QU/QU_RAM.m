...
if Xopt(t,l,1) == 0
    ...
    for tile_num = 1:64
        Q(t+l-1,tile_num) = squeeze(Q_P(t+l-1,tile_num,Xopt(t,l,tile_num)));
    end
end
...
if brunch == 0
    for i = 0:N-1
        lat = rem(i,8)+1;
        lon = fix(i/8)+1;
        UQn(t,l,i+1) = W(l)*E_tile(t-10,l,lat,lon)*Q_U(t+l-1,i+1);
%         UQn(t,l,i+1) = W(t+l-1,lat,lon)*Q_U(t+l-1,i+1);
    ...
    for i = 0:N-1
        lon = fix(i/8)+1;
        lat = rem(i,8)+1;
        UQn(t,l,i+1) = E_tile(t-10,l,lat,lon)*(Q(t+l-1,i+1)+W(l)*(Q_U(t+l-1,i+1)-Q(t+l-1,i+1)));
    end
end
