...
if brunch == 0
    for i = 0:N-1
        lon = fix(i/8)+1;
        lat = rem(i,8)+1;
        UQn(t,c_buffer,i+1) = QU_weight*W_S(t+c_buffer-1,lat,lon)*E_tile(t-10,c_buffer,lat,lon)*Q_U(t+c_buffer-1,i+1); 
%         UQn(t,c_buffer,i+1) = W(t+c_buffer-1,lat,lon)*Q_U(t+c_buffer-1,i+1);
    ...
    for i = 0:N-1
        lon = fix(i/8)+1;
        lat = rem(i,8)+1;
        UQn(t,c_buffer,i+1) = E_tile(t-10,c_buffer,lat,lon)*(Q(t+c_buffer-1,i+1)+QU_weight*W_S(t+c_buffer-1,lat,lon)*(Q_U(t+c_buffer-1,i+1)-Q(t+c_buffer-1,i+1)));
    end
end
