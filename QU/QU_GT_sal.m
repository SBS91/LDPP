...
if brunch == 0
    for i = 0:N-1
        lon = fix(i/8)+1;
        lat = rem(i,8)+1;
        UQn(t,l,i+1) = 64*tile(t+l-1,lat,lon)*W_S(t+l-1,lat,lon)*Q_U(t+l-1,i+1); 
%         UQn(t,l,i+1) = W(t+l-1,lat,lon)*Q_U(t+l-1,i+1);
    ...
    for i = 0:N-1
        lon = fix(i/8)+1;
        lat = rem(i,8)+1;
        UQn(t,l,i+1) = tile(t+l-1,lat,lon)*(Q(t+l-1,i+1)+64*W_S(t+l-1,lat,lon)*(Q_U(t+l-1,i+1)-Q(t+l-1,i+1)));
    end
end
